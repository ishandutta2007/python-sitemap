...
parser.add_argument('--videos', action="store_true", default=False, required=False, help="Add video to sitemap.xml (see https://developers.google.com/search/docs/crawling-indexing/sitemaps/video-sitemaps)")
parser.add_argument('--no-hreflang', action="store_false", default=True, required=False, dest='hreflang', help="Don't add hreflang alternate links to sitemap.xml. By default, <link rel=\"alternate\" hreflang=\"...\"> tags found on a page are added to its sitemap entry (see https://developers.google.com/search/docs/specialty/international/localized-versions#sitemap)")
...
parser.add_argument('--render-js', action="store_true", default=False, required=False, help="Render pages with headless Chromium (via Playwright) before parsing, to discover links added by client-side JS (React/Vue/Svelte SPAs). Requires Playwright (see README) or the Dockerfile.playwright image. Forces --num-workers to 1.")
